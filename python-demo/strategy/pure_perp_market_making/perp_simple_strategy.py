...
class Demo(PerpTemplate):
    ...
    def init_strategy(self):
        ...
        self.curr_duration_volume = 0.0
        self.last_duration_volume = 0.0
        self.tick = None
        ...
        if self.spread_ratio < 0:
            ...
            print(
                "According to Inj proposal 106, self-cross behavior may be banned, and can't get reward from current Trade&Earn Epoch."
            )
        ...
        if self.setting.get("start_time", None):
            self.start_time = datetime.strptime(
                self.setting["start_time"], "%Y-%m-%d %H:%M%S.%f"
            )
        else:
    ...
    def add_schedule(self):
        ...
        self.sched.add_job(self.on_timer, "interval", seconds=self.interval, id="timer")

        self.sched.add_job(
            self.close_position,
            "interval",
            seconds=int(self.re_balance_interval_hour) * 3600,
            id="re_balance_position",
        )
    ...
    async def market_making(self):
        ...
        for idx, msg in enumerate(self.msg_list):
            self.logger.info(f"msg {idx}: {msg}")

        if len(self.msg_list):
            await self.send_to_chain()

    async def send_to_chain(self):
        tx = (
            Transaction()
    ...
    def quote_bid_ask(self):
        ...
        self.logger.info("short {}btc @price{}".format(self.order_size, self.ask_price))
    ...
    async def close_position(self):
        ...
        if self.net_position > 0 and self.tick:
            self.msg_list.append(
                self.composer.MsgCreateDerivativeLimitOrder(
                    market_id=self.market_id,
                    sender=self.sender,
                    ...
                    fee_recipient=self.fee_recipient,
                    price=floor_to(
                        self.tick.bid_price_1 * (1 - 0.0001), self.tick_size
                    ),
        ...
        elif self.net_position < 0 and self.tick:
            self.msg_list.append(
                self.composer.MsgCreateDerivativeLimitOrder(
                    market_id=self.market_id,
                    sender=self.sender,
                    ...
                    fee_recipient=self.fee_recipient,
                    price=floor_to(
                        self.tick.ask_price_1 * (1 + 0.0001), self.tick_size
                    ),
                    quantity=floor_to(-self.net_position, self.step_size),
                    is_reduce_only=True,
                    is_buy=True,
